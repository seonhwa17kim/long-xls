...
def _ensure_samples():
    """Generate test files in samples/ if they don't exist."""
    if not (SAMPLES / "test_100k_rows.xls").exists():
        SAMPLES.mkdir(parents=True, exist_ok=True)
        from tests.generate_test_xls import generate_long_xls, generate_encodings_test
        generate_long_xls(SAMPLES / "test_100k_rows.xls", num_rows=100_000)
        generate_long_xls(SAMPLES / "test_200k_rows.xls", num_rows=200_000)
        generate_encodings_test(SAMPLES / "test_70k_cp949.xls")
...
class TestParse100k:
    def test_row_count(self):
        sheet = parse(SAMPLES / "test_100k_rows.xls", encoding="utf-8")
...
class TestParse200k:
    def test_row_count(self):
        sheet = parse(SAMPLES / "test_200k_rows.xls", encoding="utf-8")
...
class TestParse70kCP949:
    def test_row_count(self):
        sheet = parse(SAMPLES / "test_70k_cp949.xls", encoding="cp949")
...
class TestScan:
    def test_scan_100k(self):
        info = scan(SAMPLES / "test_100k_rows.xls")
